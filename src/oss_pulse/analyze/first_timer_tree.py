...
def _wilson_lower(n_success: int, n_total: int, z: float = 1.96) -> float:
    ...
    Penalizes small samples: a repo with 5/5 merges scores lower than
    one with 80/100, even though the raw rate is higher.
    """
    if n_total == 0:
        return 0.0
    p = n_success / n_total
...
def _top_repos_for_language(
    ...
    Ranked by Wilson score lower bound to avoid small-sample bias.
    """
    ...
    repo_agg["wilson_score"] = repo_agg.apply(
        lambda r: _wilson_lower(int(r["ft_merged"]), int(r["ft_prs"])), axis=1
    )
    repo_agg = repo_agg.sort_values("wilson_score", ascending=False).head(n)
    ...
    for repo, row in repo_agg.iterrows():
        results.append({
            ...
            "stars": int(row["stars"]),
            "wilson_score": round(float(row["wilson_score"]), 3),
        })
    return results
